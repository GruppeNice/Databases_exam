...
import com.example.hospital_db_backend.model.mysql.*;
import com.example.hospital_db_backend.model.types.*;
import com.example.hospital_db_backend.repository.*;
...
public class BulkDataSeederService {
    ...
    private List<Hospital> generateHospitals(int count) {
        ...
        // Wards are already generated and associated in seedBulkData method
        // Just need to refresh the hospitals to get the wards relationship
        return hospitals;
    }
    ...
    private List<Ward> generateWardsForHospital(Hospital hospital) {
        ...
        Set<WardType> usedTypes = new HashSet<>(); // Track used ward types to ensure uniqueness

        for (int i = 0; i < wardCount; i++) {
            Ward ward = new Ward();
            ward.setWardId(UUID.randomUUID());
            
            // Ensure unique ward type per hospital
            WardType wardType;
            int attempts = 0;
            do {
                wardType = wardTypes[ThreadLocalRandom.current().nextInt(wardTypes.length)];
                attempts++;
            } while (usedTypes.contains(wardType) && attempts < 100);
            usedTypes.add(wardType);
            
            ward.setType(wardType);
